...
# "old directives !/usr/bin/env python3.6"
...
HFU_LV_REQUEST_URL =  "http://35.224.101.35:2020/collector/datafromresource/1"  #"http://localhost:2020/collector/datafromresource/1"
...
UPDATING_URL = "http://35.224.101.35:2020/collector/data/1"   #"http://localhost:2020/collector/data/1"   #
...
def create_threads0(num_of_threads):
    real_results0 = []

    current_average_response_time_value0 = 0
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_of_threads) as executor:
        main_start = datetime.datetime.now()
                
        for arg in range(num_of_threads):
        ...
        for future in future_list:
            try: 
                ...
                if single_result is not None:
                    real_results0.append(single_result)
            except Exception as e:
                print("Exception in futures.ThreadPoolExecutor: ", e)
       
        #response_time_set.append(real_results0)
        
        main_end = datetime.datetime.now()
        main_diff = millis_interval(main_start, main_end)


    real_results_size = len(real_results0)
    if real_results_size > 0:
        for value in real_results0:
            current_average_response_time_value0 = statistics.mean(real_results0)

    else:
        current_average_response_time_value0 = 1
          
    #average_response_time_set.append(current_average_response_time_value)
    print("current_average_response_time_value0 ====  ====  ====  ====  ========== ", current_average_response_time_value0)
    

# This method is responsible for ....
def create_threads1(num_of_threads):
    real_results1 = []
    current_average_response_time_value1  = 0
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_of_threads) as executor:
        main_start = datetime.datetime.now()
        ...
        for future in future_list:
            try: 
                ...
                single_result = future.result()
                if single_result is not None:
                    real_results1.append(single_result)
            except Exception as e:
                print("Exception in futures.ThreadPoolExecutor: ", e)
       
        #response_time_set.append(real_results1)
        
        main_end = datetime.datetime.now()
        main_diff = millis_interval(main_start, main_end)


    real_results_size = len(real_results1)
    if real_results_size > 0:
        for value in real_results1:
            current_average_response_time_value1 = statistics.mean(real_results1)

    else:
        current_average_response_time_value1 = 1
          
    #average_response_time_set.append(current_average_response_time_value)
    print("current_average_response_time_value1 ====  ====  ====  ====  ========== ", current_average_response_time_value1)
...
if __name__ == "__main__": 
    ...
    for round in NUM_THREADS_ARRAY:
        for i in range(NUMBER_OF_SEQ_ROUNDS):
            if (thread_group_flag == 0):
                time_start = datetime.datetime.now()
                time.sleep(1)
                
                thread2 = threading.Thread(target = create_threads0,  args = (round,))
                thread2.start()
                #create_threads0(round) 
 
                print("==== NUMBER OF THREADS ======(thread group 0)============: ", round)
                print("==== ITERATION ==========================================: ", i)
                time_end = datetime.datetime.now()
                main_diff = millis_interval(time_start, time_end)
                print("BURST TIME ========================== ", main_diff)
                thread_group_flag = 1
            else:
                time_start = datetime.datetime.now()
                time.sleep(1)

                thread3 = threading.Thread(target = create_threads1,  args = (round,))
                thread3.start()
                #create_threads1(round)  

                print("==== NUMBER OF THREADS ======(thread group 1)============: ", round)
                print("==== ITERATION ==========================================: ", i)
                time_end = datetime.datetime.now()
                main_diff = millis_interval(time_start, time_end)
                print("BURST TIME ========================== ", main_diff)
