...
UPDATE_TIME = 1 		# time interval for updating the database  (in seconds)
NOTIFY_STACKDRIVER_TIME = 10


NUM_THREADS_ARRAY = [1,2,4,8,16,32,33,34,35,36,37,38,39,40,45,50,55,60,65]#,66,67,68,69,70]#,71,72,73,74,75,76,77,78,79,80]#, 20, 50, 100, 150, 200]#

#HFU_LV_REQUEST_URL = "http://35.224.101.35:2020/collector/datafromresource/1"
#LFU_HV_REQUEST_URL = "http://35.184.139.114:2020//collector/resources/1/data"
HFU_LV_REQUEST_URL = "http://35.184.139.114:2020/collector/datafromresource/1"
#HFU_LV_REQUEST_URL = "http://35.224.101.35:2020/collector/datafromresource/1"
LFU_HV_REQUEST_URL = "http://35.184.139.114:2020//collector/resources/1/data"
#LFU_HV_REQUEST_URL = "http://35.224.101.35:2020//collector/resources/1/data"
STACKDRIVER_URL = "http://35.239.239.83:8081/"
UPDATING_URL = "http://35.184.139.114:2020/collector/data/1"
...
#def make_request(): 
    try:
        ...
        #resp = req.get(LFU_HV_REQUEST_URL)
...
#def make_request(): 
    try:
        ...
        resp_data = ResponseData()
        resp_data.url = LFU_HV_REQUEST_URL#HFU_LV_REQUEST_URL
        resp_data.response_content = resp.content
        resp_data.response_status_code = resp.status_code
        resp_data.response_time = diff
        #return resp_data
...
if __name__ == "__main__": 
        ...
        if round >= 40:
            ...
            for i in range(NUMBER_OF_SEQ_ROUNDS):
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=round) as executor:
                    ...
                    #print("RESULTS = ======================================= ==== make_request_hfu_lv ")#, results[0])
                    #for arg in range(round):
                        #future = executor.submit(make_request_hfu_lv, arg)
                    
                    print("RESULTS = ======================================= ==== make_request_LFU_HV ")#, results[0])
                    for arg in range(round):
                        future = executor.submit(make_request_lfu_hv, arg)
    ...
    f = open("MIXED_WITHPAL_RESULTS/Response_time_mixed_withPAL(0).data","a+")
    ...
    f.write("WORKLOAD: HFU_LV AND HFU_HV;  \n")
