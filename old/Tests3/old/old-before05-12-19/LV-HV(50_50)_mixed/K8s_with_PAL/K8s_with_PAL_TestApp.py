...
def make_request(id):
    ...
    #resp = req.get("http://35.184.118.160:2020//collector/datafromresource/1")
    #resp = req.get("http://35.184.118.160:2020//collector/resources/1/data")
    
    if id%2 == 0 :
        resp = req.get("http://35.232.10.207:2020//collector/resources/1/data")
        flag = 0
        ...
        resp = req.get("http://35.232.10.207:2020//collector/datafromresource/1")
        flag = 1
        end = datetime.datetime.now()
        diff = millis_interval(start, end)
        #diff = int(delta.total_seconds() * 1000) # milliseconds
